...
int aflag,length_of_file, pflag, tflag;
char* tgaFileName;
int testCase;
...
void set_flags(int argc, char * argv[])
{
    ...
    while ((c = getopt (argc, argv, "::af:h:p:t:")) != -1)
        switch (c)
    {
        case 'a':
        ...
        case 'f':
            ...
            printf ("length is %li", strlen(argv[optind -1]));
            tgaFileName = (argv[optind - 1]);
            length_of_file = (int)(strlen(argv[optind -1]));
...
int main(int argc, char *argv[])
{
    ...
    if( aflag && tflag){
        printf("aflag and tflag are incompatible together. Exiting program\n");
        exit(EXIT_SUCCESS);
    }
    run_test_case (tgaFileName, testCase, pflag, length_of_file);
